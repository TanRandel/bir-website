...
if (dfMessenger) {
  dfMessenger.addEventListener('df-messenger-loaded', function (event) {
    ...
    $('<button>', { class: 'dfmessenger', title: this.getAttribute('chat-title') })
      .append('<span class="material-icons">support_agent</span>')
      .insertBefore($('.chat-container'));

    $('.dfmessenger').click(function (event) {
      event.preventDefault();
      $('.dfmessenger .material-icons').text(
        dfMessenger.shadowRoot.getElementById('df-chat-wrapper').shadowRoot.querySelector('.chat-wrapper.chat-open')
          ? 'support_agent'
          : 'south_east'
      );
    ...
    $(this.shadowRoot.getElementById('df-chat-wrapper').shadowRoot).prepend(
      $('<style>').text(
        '.chat-wrapper { ...your !important style rules... } .chat-wrapper.chat-closed { display: none; } df-message-list { ...your !important style rules... } @media screen and ( max-width: 500px ) { .chat-wrapper.chat-open { ...your !important style rules... } }'
      )
    );
    ...
    new MutationObserver(function () {
      ...
      if (dialog) {
        ...
        if (!$(list).find('> style').length)
          $(list).prepend(
            $('<style>').text('#messageList .message { ...your !important style rules... }')
          );
        ...
        if (!$(input).find('> style').length)
          $(input).prepend(
            $('<style>').text('.input-box-wrapper { ...your !important style rules... } #sendIcon:hover { fill: ...your !important color...; }')
          );
          ...
          $(list.getElementById('messageList').querySelectorAll('df-chips')).each(function () {
            if (!$(this.shadowRoot).find('> style').length)
              $(this.shadowRoot).prepend(
                $('<style>').text(
                  '.df-chips-wrapper .chip { ...your !important style rules... } .df-chips-wrapper .chip:hover { ...your !important style rules... } .df-chips-wrapper a, button { ...your !important style rules... } .df-chips-wrapper .chip:hover button { ...your !important style rules... }'
                )
              );
          });
